...
find_extra_block_dev() {
  ...
  if [ -z "${extra_dev}" ]; then
    ...
    sudo apt-get install -y -q targetcli-fb open-iscsi
    ...
    sudo targetcli /backstores/fileio \
      create disk1 ~/iscsi-disk.img 20G
    sudo targetcli /iscsi create "${tgt}"
...
prepare_disk() {
  ...
  sudo wipefs --all --force "/dev/${BLOCK}" || true
  sudo sgdisk --zap-all "/dev/${BLOCK}" || true
  sudo dd if=/dev/zero of="/dev/${BLOCK}" \
    bs=1M count=100 oflag=direct || true
